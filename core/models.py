...
class Page(wg_models.Page):
    ...
    promote_panels = [
        panels.MultiFieldPanel(
            wg_models.Page.promote_panels, "Common page configuration"
        ),
...
class PageRelatedLink(wg_models.Orderable):
    page = mc_fields.ParentalKey(
        Page, on_delete=models.CASCADE, related_name="related_links"
    )
    name = models.CharField(max_length=255)
    url = models.URLField()

    panels = [panels.FieldPanel("name"), panels.FieldPanel("url")]
